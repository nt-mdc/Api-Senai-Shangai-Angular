...
async function createSpot(data, files, userId) {
  ...
  const requiredFields = ['name', 'description', 'district', 'category', 'latitude', 'longitude', 'requires_ticket']
  ...
  if (isNaN(latitude) || latitude < 30.70 || latitude > 31.53) {
  ...
  const spot = await prisma.turismSpot.create({
    data: {
      ...
      latitude,
      longitude,
      requires_ticket: data.requires_ticket === 'true' || data.requires_ticket === true,
      created_by: userId,
      images: {
...
async function updateSpot(id, data, files) {
  ...
  if (data.requires_ticket !== undefined) {
    updateData.requires_ticket = data.requires_ticket === 'true' || data.requires_ticket === true
  }
