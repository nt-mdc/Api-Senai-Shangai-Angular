...
exports.list = async (req, res, next) => {
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.electronicCategory.findMany({ orderBy: { name: 'asc' }, skip, take }),
      prisma.electronicCategory.count()
    ])
...
exports.getById = async (req, res, next) => {
  try {
    const category = await prisma.electronicCategory.findUnique({
...
exports.create = async (req, res, next) => {
  try {
    ...
    const category = await prisma.electronicCategory.create({ data: { name, description: description ?? null, image } })
...
exports.update = async (req, res, next) => {
  try {
    ...
    const existing = await prisma.electronicCategory.findUnique({ where: { id } })
    ...
    const category = await prisma.electronicCategory.update({ where: { id }, data })
...
exports.remove = async (req, res, next) => {
  try {
    ...
    if (!existing) return res.status(404).json({ error: 'Categoria não encontrada' })
    await deleteBlob(existing.image)
    await prisma.electronicCategory.delete({ where: { id } })
